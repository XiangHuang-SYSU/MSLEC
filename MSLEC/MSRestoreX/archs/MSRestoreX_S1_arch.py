...
class VQVAE_Prior(nn.Module):
    def __init__(self, patch_nums = (1, 4, 8, 16),vae_ckpt='your-path/VAR-main/vae_ch160v4096z32.pth'):
        ...
        #self.resize = Resize((256,256), interpolation=BICUBIC)
...
class MSRestoreXS1(nn.Module):
    def __init__(self, 
        ...
        super(MSRestoreXS1, self).__init__()
        ...
        self.VAEPrior = VQVAE_Prior(patch_nums=patch_nums,vae_ckpt=vae_ckpt)
    def forward(self, x, gt):
        if self.training:
            S1_prior_fea, S1_prior_encoder = self.VAEPrior(gt)
            sr = self.G(x, S1_prior_fea,S1_prior_encoder)
            return sr,S1_prior_fea,S1_prior_encoder
        else:
            padding_num_h = 0
            padding_num_w = 0
            n,c,h,w = x.shape
